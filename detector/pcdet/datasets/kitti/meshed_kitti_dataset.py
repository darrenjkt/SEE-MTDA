...
class MeshedKittiDataset(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading MeshedKittiDataset dataset')
        kitti_infos = []
        ...
        if self.logger is not None:
            self.logger.info('Total samples for MeshedKittiDataset dataset: %d' % (len(self.kitti_infos)))
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        if self.dataset_cfg.get('FILTER_MIN_POINTS_IN_GT', False):
            ...
            for info in self.kitti_infos:
                annos = info['annos']
                dontcare_mask = (annos['name'] != 'DontCare')
                minpts_mask = (annos['num_points_in_gt'] >= self.dataset_cfg.FILTER_MIN_POINTS_IN_GT)
                for key in annos.keys():
                    if key == 'gt_boxes_lidar':
                        combined_mask = minpts_mask[dontcare_mask]
                        annos[key] = annos[key][combined_mask]
                    else:
