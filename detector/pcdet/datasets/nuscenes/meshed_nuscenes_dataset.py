...
class MeshedNuScenesDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        if not dataset_cfg.CUSTOM:
    ...
    def include_nuscenes_data(self, mode):
        self.logger.info('Loading MeshedNuScenes dataset')
        nuscenes_infos = []
        ...
        self.logger.info('Total samples for MeshedNuScenes dataset: %d' % (len(nuscenes_infos)))
    ...
    def get_meshed_lidar(self, index):
        info = self.infos[index]
        meshed_lidar_file = self.root_path / info['meshed_lidar_path']

        assert meshed_lidar_file.exists(), f"No file at: {meshed_lidar_file}"
        file_extension = str(meshed_lidar_file).split('.')[-1]
        if file_extension == "pcd":
            pcd = o3d.io.read_point_cloud(str(meshed_lidar_file))
            return np.asarray(pcd.points, dtype=np.float32)

        elif file_extension == "bin":
            # This is only for the seg-label approach, fields are [x,y,z,bg_label, car_label] where label = 1 or 0
            points = np.fromfile(str(meshed_lidar_file), dtype=np.float32, count=-1).reshape([-1, 5])
            return points
        else:
            print(f"Error: {file_extension} lidar file extension not supported")
            return None
    ...
    def __getitem__(self, index):
        ...
        points = self.get_meshed_lidar(index)
