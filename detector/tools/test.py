...
def parse_config():
    ...
    # Assumes you use relative format to specify cfg file (has to be from tools)
...
def get_no_evaluated_ckpt(ckpt_dir, ckpt_record_file, args):
    ...
    ckpt_list = sorted(ckpt_list, key=lambda x: int(x.split('checkpoint_epoch_')[-1].split('.')[0]))
...
def main():
    ...
    duplicate_test_cfg = str(eval_output_dir / Path(args.cfg_file).name)
    shutil.copy(args.cfg_file, duplicate_test_cfg)
