...
def segm2json(result, class_ids, score_thresh=0.5, mask=True):
    """
    ...
    # Keep only the results with > threshold confidence
    ...
    for label_id in range(len(det)):
        ...
        score = [bbox[4] for bbox in bboxes]

        for i in range(bboxes.shape[0]):
            data = dict()
            if label_id in class_ids:      # 0:person, 1:bicycle, 2:car       
                data['category_id'] = label_id           
            else:
                continue

            data['bbox'] = xyxy2xywh(bboxes[i])
            data['score'] = float(score[i])
            if mask:
